...
import gearIco from "@/public/imgs/gear.ico";
type Props = {
  title: string;
  link: string;
  imgSrc: string;
...
export default function Card({ title, link, imgSrc }: Props) {
  return (
      <div className="w-full h-72 grid items-center relative">
        <Link href={link} className="peer group w-full h-1/3 border-[6px] border-x-amber-700 border-y-white z-[2] flex justify-center items-center relative bg-fuchsia-950 hover:bg-fuchsia-950/10 hover:h-full transition-all duration-1000">
