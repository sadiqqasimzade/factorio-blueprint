...
export default function ImgToLampBlueprintConvertor(color_indexes: number[][], quality: number): Blueprint {
  const width = color_indexes.length
  const height = color_indexes[0].length
  const wires: TBpWire[] = []
  const mainEntities: BpEntity[] = CreateScreen(width, height, wires, quality)
